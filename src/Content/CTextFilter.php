...
class CTextFilter
{
    use \Anax\TConfigure,
		...
		/**
		 * Kortar ner en text och lägger till "läs mer"
		 */
		public function shorten($string, $length, $link)
		{
			$string = strip_tags($string);
			
			if(strlen($string) > $length){
				$stringCut = substr($string, 0, $length);
				$string = substr($stringCut, 0, strrpos($stringCut, ' ')).'... <br/><br/><a class="readmore" href="' . strip_tags($link) . '">Läs mer...</a>';
			}
			return $string;
		}
